 ...
 * builder publishes arch-specific portable exes as
 * Summer-Breeze-GUI-<version>-<arch>.exe for this app (see the
...
export async function getAppLatestRelease(): Promise<AppUpdateInfo> {
  ...
  const name = `Summer-Breeze-GUI-${version}-${archSuffix}.exe`
